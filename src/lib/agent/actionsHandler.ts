...
export const postAgentAction: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (
      ...
      if (action !== 'admin_scout_list_searches' && action !== 'admin_scout_delete_search') {
        const rate = checkAdminScoutRateLimit(
          request,
          scoutSessionId,
          action === 'admin_scout_chat' ? 'admin_scout_chat' : 'admin_scout_search'
        );
    ...
    if (action === 'admin_scout_chat') {
      ...
      console.info('[admin-squad] chat:start', {
        scoutSessionId,
      ...
      const result = await runFounderAgentChat({
        ...
        jobId: payload?.startFresh === true ? null : payload?.opportunityId || null,
        message: userText,
      });
      const searchTool = [...(result.toolCalls || [])].reverse().find((tool) => tool.name === 'search_talent');
      const searchResult = searchTool?.result as { search?: { uiBlock?: unknown }; uiBlock?: unknown; shortlist?: unknown } | undefined;
      const searchUiBlock = searchResult?.search?.uiBlock || searchResult?.uiBlock || null;
      console.info('[admin-squad] chat:done', {
        scoutSessionId,
        opportunityId: result.job?.id || null,
        toolCalls: (result.toolCalls || []).map((tool) => tool.name),
        hasSearchTool: Boolean(searchTool),
        hasSearchUiBlock: Boolean(searchUiBlock),
      });

      return ok({
        ...result,
        opportunity: result.job,
        uiBlocks: searchUiBlock ? [searchUiBlock] : [],
        shortlist: searchResult?.shortlist,
      });
    }
    ...
    if (action === 'admin_scout_search') {
      ...
      try {
        console.info('[admin-squad] search:start', {
          scoutSessionId,
          opportunityId,
          searchMode,
        });
        ...
        const result = await handleJobAction(founderIdentity, {
          action: 'search_talent',
          payload: { jobId: opportunityId, searchMode },
        });
        const opportunity = await Opportunity.findOne({
          _id: opportunityId,
          founderEmail: identity.founderEmail,
        ...
        const fullCandidates = shortlistDoc
          ? await buildAdminCandidatesForShortlist(shortlistDoc, opportunity || (result as any).job, {
              BuilderProfile,
              ProjectRecord,
              MatchRecord,
            })
          : [];
        const search = (result as any).search || null;
        console.info('[admin-squad] search:done', {
          scoutSessionId,
          opportunityId,
          candidates: fullCandidates.length,
          strong: fullCandidates.filter((c) => c.matchLabel === 'Strong Match').length,
          skipped: Boolean(search?.skipped),
        });

        return ok({
          message: (result as any).message,
          opportunity: opportunity || (result as any).job,
          shortlist: {
            totalMatches: fullCandidates.length,
            strongMatchCount: fullCandidates.filter((c) => c.matchLabel === 'Strong Match').length,
            candidates: fullCandidates,
          },
          uiBlocks: search?.uiBlock ? [search.uiBlock] : [],
          meta: { model: hasOpenRouterConfig() ? getOpenRouterChatModel() : 'deterministic-fallback' },
        });
      } catch (error) {
        console.error('[admin-squad] search:error', {
          scoutSessionId,
          opportunityId,
