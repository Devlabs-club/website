...
import { Bell, Search, Settings, UsersRound } from "lucide-react";

export const FounderRail: React.FC<{ onLogout: () => void; initial: string; active?: "sourcing" | "settings" }> = ({
  onLogout,
  initial,
  active = "sourcing",
}) => (
  <aside className="flex min-h-screen w-20 shrink-0 flex-col items-center border-r border-[#ece7e1] bg-white py-6">
    ...
    <div className="flex flex-col items-center gap-4">
      <button
        type="button"
        className="relative grid h-11 w-11 place-items-center rounded-xl text-black/35 transition hover:bg-[#fdfaf7] hover:text-black/70"
        title="Notifications"
      >
        <Bell className="h-5 w-5" />
        <span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-[#ec9149]" />
      </button>
      <a
