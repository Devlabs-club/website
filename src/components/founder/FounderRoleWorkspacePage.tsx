...
const HIRED_STATUSES = new Set(["offer", "hired"]);
...
const FounderRoleWorkspaceInner: React.FC<{ roleId: string }> = ({ roleId }) => {
  ...
  const [rightPane, setRightPane] = useState<"editor" | Bucket | "profile">("editor");
  ...
  /** Open a builder's full profile, replacing the tab content. */
  ...
  const rejectCandidate = (builderId: string) => {
  ...
  const showTabs = recommendations.length > 0 || (rightPane !== "editor" && rightPane !== "profile");
    ...
    <div className="flex min-h-screen bg-white">
            ...
            {rightPane === "editor" ? (
              ...
              <div className="mx-auto max-w-3xl">
                ...
                {profileLoading || !profile ? (
                  ...
                  <div className="rounded-[28px] border border-[#ece7e1] bg-white p-6 shadow-[0_1px_3px_rgba(16,24,40,0.05),0_10px_30px_rgba(16,24,40,0.05)]">
                ...
                {searching && buckets.recommended.length === 0 ? (
                  ...
                  buckets.recommended.map((rec) => (
                    <RecommendationCard
                      ...
                      onOpenProfile={() => void openProfile(rec.builderId)}
                      onInvite={() => void invite(rec.builderId)}
                      onReject={() => rejectCandidate(rec.builderId)}
                    />
                  ))
                )}
              </div>
              ...
              <PipelineTable
                ...
                renderRow={(rec) => {
                  ...
                  return (
                    <>
                      <BuilderCell rec={rec} onOpenProfile={() => void openProfile(rec.builderId)} />
                ...
                renderRow={(rec) => {
                  ...
                  return (
                    <>
                      ...
                      <td className="py-4 text-sm text-black/55">{TRIAL_STATUS_LABELS[statusKey] || statusKey}</td>
                      <td className="py-4">
                        <button
                          type="button"
                          onClick={() =>
              ...
              <PipelineTable
                ...
                renderRow={(rec) => (
                  <>
                    ...
                    <td className="py-4 text-sm text-black/55">{rec.matchStatus === "offer" ? "Offer extended" : "Hired"}</td>
                    <td className="py-4">
                      <button
                        type="button"
      ...
      {chatOpen && (
        <div className="fixed inset-0 z-40 flex justify-end bg-black/30" onClick={() => setChatOpen(false)}>
          <div
            className="flex h-full w-full max-w-md flex-col border-l border-[#ece7e1] bg-white shadow-[0_1px_3px_rgba(16,24,40,0.05),0_24px_70px_rgba(16,24,40,0.15)]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-[#ece7e1] p-5">
              <div>
...
const RecommendationCard: React.FC<{
  ...
  return (
    <article className="rounded-[28px] border border-[#ece7e1] bg-white p-6 shadow-[0_1px_3px_rgba(16,24,40,0.05),0_10px_30px_rgba(16,24,40,0.05)]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex min-w-0 gap-4">
          <Avatar name={rec.name} avatarUrl={rec.avatarUrl} />
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-1.5">
              <button type="button" onClick={onOpenProfile} className="font-semibold text-black hover:underline">
                {rec.name}
              </button>
              {verified && <BadgeCheck className="h-4 w-4 text-[#ec9149]" />}
            </div>
            <p className="mt-0.5 truncate text-sm text-black/55">{rec.headline || rec.bio}</p>
            {rec.location && <p className="mt-0.5 text-xs text-black/40">{rec.location}</p>}
          </div>
        </div>
        <div className="flex shrink-0 flex-col items-end gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {links.portfolio && (
              <a href={links.portfolio} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-medium text-black/55 hover:text-black">
                <Globe className="h-3.5 w-3.5" /> Portfolio
              </a>
            )}
            {links.resume && (
              <a href={links.resume} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-medium text-black/55 hover:text-black">
      ...
      {rec.whyTheyMatch && (
        <div className="mt-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-black/40">Why You Match</p>
          <p className="mt-1 text-sm leading-relaxed text-black/70">{rec.whyTheyMatch}</p>
        </div>
      )}

      {(rec.experiences || []).length > 0 && (
        <div className="mt-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-black/40">Experience</p>
          <div className="mt-2 flex flex-wrap gap-2">
            {(rec.experiences || []).map((exp, index) => (
              <span key={index} className="rounded-full border border-[#ece7e1] bg-[#fdfaf7] px-3 py-1.5 text-xs text-black/60">
      ...
      {(rec.projects || []).length > 0 && (
        <div className="mt-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-black/40">Project</p>
          <div className="mt-2 flex flex-wrap gap-2">
            {(rec.projects || []).map((project) => {
              ...
              return href ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1 rounded-full border border-[#ece7e1] bg-[#fdfaf7] px-3 py-1.5 text-xs text-black/60 hover:bg-[#f3ede4]"
                >
                  {project.projectName} <ExternalLink className="h-3 w-3" />
                </a>
              ) : (
                <span key={project._id} className="rounded-full border border-[#ece7e1] bg-[#fdfaf7] px-3 py-1.5 text-xs text-black/60">
                  {project.projectName}
                </span>
              );
            })}
      ...
      {(rec.topSkills || []).length > 0 && (
        <div className="mt-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-black/40">Top Skills</p>
          <div className="mt-2 flex flex-wrap gap-2">
            {(rec.topSkills || []).map((skill) => (
              <span key={skill} className="rounded-full border border-[#ece7e1] px-3 py-1.5 text-xs text-black/55">
                {skill}
              </span>
            ))}
          </div>
      ...
      <div className="mt-5 flex items-center justify-between border-t border-[#ece7e1] pt-4">
        <button type="button" onClick={onOpenProfile} className="inline-flex h-9 items-center rounded-xl bg-[#f3ede4] px-3 text-xs font-medium text-black hover:bg-[#ece3d5]">
          View full profile
        </button>
        <div className="flex gap-2">
          <button
            type="button"
            onClick={onReject}
            className="inline-flex h-9 items-center rounded-xl border border-[#ece7e1] px-3 text-xs font-semibold text-black hover:bg-[#fdfaf7]"
          >
            Reject
          </button>
          <button
            ...
            className="inline-flex h-9 items-center justify-center gap-2 rounded-xl bg-[#ec9149] px-3 text-xs font-semibold text-white hover:bg-[#dd7f36] disabled:opacity-50"
          >
